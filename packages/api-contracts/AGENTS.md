...
- For Payload page composition, own canonical discriminators, normalized render DTOs, and framework-neutral authoring limits here. Raw Payload REST envelope schemas remain web-owned; generated Payload types are not runtime validators. Follow the `payload-block-renderer` skill at `.agents/skills/payload-block-renderer/`.
