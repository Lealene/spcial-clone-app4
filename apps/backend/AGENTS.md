...
- **Page blocks follow the renderer standard.** Use the `payload-block-renderer` skill at `.agents/skills/payload-block-renderer/` when available. Reuse shared enabled/anchor/link/media fields and keep authoring limits aligned with `@mvp-realty/api-contracts`.
...
- Globals are `site-settings`, `header`, `footer`, and `privacy-policy`. The privacy policy is a global rather than a Pages doc — one canonical legal document, non-editable slug, no page-builder layout. Seed placeholder copy with `seed:privacy-policy`; `privacy-policy` is in `Pages.reservedSlugs` so a CMS page cannot shadow the static route.
- **`select` fields are real Postgres enums.** Adding an option needs an `ALTER TYPE ... ADD VALUE` migration — TypeScript will not catch its absence and the insert fails at the DB layer. `areas.amenities[].icon` derives its options from `COMMUNITY_AMENITY_ICONS` in `@mvp-realty/api-contracts`; extend the contract, not a local copy.
- `featuredResidences.manualListings` is deprecated and unread — the rail renders listings flagged `isFeatured`. Do not wire it back in; it exists only until its drop migration runs.
