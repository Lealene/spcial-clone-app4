...
export function Testimonials({ block }: { block: TestimonialsBlock }) {
  const total = block.stories.length;
  ...
  const play = useCallback(() => {
    if (!block.carouselAutoPlay || reduced.current || timer.current || total === 0) return;
    timer.current = setInterval(() => setIndex((i) => (i + 1) % total), block.carouselIntervalMs);
  }, [block.carouselAutoPlay, block.carouselIntervalMs, total]);
              ...
              >
                {block.stories.map((t, i) => (
                  <button
                    ...
                    aria-label={t.tabAriaLabel ?? t.name}
                    onClick={() => goto(i)}
                    className={cn(
