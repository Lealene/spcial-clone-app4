...
import { heroCommunities } from '@/data/communities';

export function CommunitiesStrip() {
  return (
    <div className="bg-primary relative z-20 bg-[linear-gradient(180deg,rgba(255,255,255,.04),transparent_40%)] text-white/90">
      <Container className="grid sm:grid-cols-3">
        {heroCommunities.map((c) => (
          <Link
            key={c.slug}
            href={`/communities/${c.slug}`}
            className="group flex items-center gap-[19px] border-t border-white/10 px-[clamp(20px,3vw,44px)] py-[30px] transition-colors hover:bg-white/[0.035] sm:border-t-0 sm:border-l sm:first:border-l-0"
          >
