...
- **Listing facets: `community` is CMS-driven, the other three are not.** `type`, `status` and `features` mirror closed Payload `select` enums declared in `@mvp-realty/api-contracts` and are validated by membership, so their options stay in `STATIC_FACET_OPTIONS` (`src/lib/listings/filters.ts`). `community` is `listing.area.slug` — an open-ended relationship — so its options come from `getCommunityFilterOptions()` and are passed down as props, and `?community=` is validated by **slug shape, not membership**. Validating it against a fixed list silently drops URLs for any area added since the last deploy.
- `getCommunityFilterOptions()` deliberately does **not** filter to `kind: community`. Listings also attach to city-kind areas (Fort Myers Beach, Bonita Springs); filtering by kind makes every listing in one unfilterable.
- **One sitemap.** `app/sitemap.ts` carries pages, communities and listings, so it inherits the 900s listings window. There is no sitemap index or shard route — see `docs/schema/structured-data.md` for why, and for the 20,000-doc pagination ceiling in `lib/cms/sitemap.ts`.
- `/privacy-policy` is a static route backed by the `privacy-policy` Payload global, not a Pages doc. The slug is reserved on both sides (`app/[slug]/page.tsx`, backend `Pages.ts`) so a CMS page cannot shadow it. Its body renders through `lib/cms/rich-text-html.ts` — the lossy `rich-text.ts` flattener drops the headings and lists a legal document needs.
